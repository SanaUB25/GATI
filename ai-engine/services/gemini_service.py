...
class GeminiService:
    ...
    def explain_plan(self, evidence: dict, question: str | None = None) -> dict:
        prompt = {'instruction': 'Explain the supplied, already-calculated plan evidence in plain language. Do not calculate, optimize, simulate, invent values, or recommend changes beyond the evidence.', 'evidence': evidence, 'question': question}
        if not self.api_key:
