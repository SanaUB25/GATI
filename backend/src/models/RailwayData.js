...
const scenarioSchema = new mongoose.Schema({ scenarioId: { type: String, unique: true, required: true, index: true }, trainDelay: { type: Number, min: 0, required: true }, maintenanceOverrun: { type: Number, min: 0, required: true }, trafficFactor: { type: Number, min: 0, required: true }, assetRisk: { type: Number, min: 0, required: true } }, { timestamps: true });
