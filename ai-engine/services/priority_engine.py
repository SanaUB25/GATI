...
def priority_factors(criticality: float, severity: float, overdue_days: float, weather_risk: float,
                     asset_importance: float, season: str = 'Other', section_importance: float = 1) -> dict:
    """Deterministic, auditable RiskClock score. Values are intentionally bounded."""
    values = [criticality, severity, overdue_days, weather_risk, asset_importance, section_importance]
...
def calculate_priority(criticality: float, severity: float, overdue_days: float, weather_risk: float,
                       asset_importance: float, season: str = 'Other', section_importance: float = 1) -> float:
    return priority_factors(criticality, severity, overdue_days, weather_risk, asset_importance, season, section_importance)['score']
