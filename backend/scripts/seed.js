...
try {
  ...
  counts.scenarios = await insertMissing(Scenario, scenarioRows.map((row) => ({ scenarioId: row.scenario_id, trainDelay: number(row.train_delay), maintenanceOverrun: number(row.maintenance_overrun), trafficFactor: number(row.traffic_factor), assetRisk: number(row.asset_risk) })), 'scenarioId');
  counts.maintenance = await insertMissing(MaintenanceTask, maintenanceRows.map((row) => ({ sourceId: row.id, sourceSystem: row.department === 'Engineering' ? 'TMS' : row.department === 'Signalling' ? 'SMMS' : 'TDMS', department: department[row.department], corridorId: 'NDLS-KKDE', assetId: row.section, durationMin: number(row.duration), dueAt: new Date(`2026-10-01T00:00:00Z`).getTime() + number(row.earliest_start) * 60_000, severity: number(row.severity), criticality: number(row.urgency), resources: [row.department], status: 'OPEN', priorityScore: Math.min(100, number(row.severity) * 12 + number(row.urgency) * 8 + (row.overdue === 'True' ? 15 : 0)) })), 'sourceId');
